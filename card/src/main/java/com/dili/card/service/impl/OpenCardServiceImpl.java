...
public class OpenCardServiceImpl implements IOpenCardService {
    ...
    public OpenCardResponseDto openCard(OpenCardDto openCardInfo) {
        ...
        // 保存全局操作交易记录 开卡工本费
        saveSerialRecord(openCardInfo, accountId, serialNo, tradeResponseDto.getWhen());
        // 发送MQ通知
        sendMQ(openCardInfo);
        //创建员工信息，附加功能，不能影响主流程
     ...
     * 生成全局日志
     */
    private void saveSerialRecord(OpenCardDto openCardInfo, Long accountId, String serialNo, LocalDateTime operateTime) {
...
//	}
    ...
    private void registerEmployee(OpenCardDto openCardInfo, Long accountId) {
        ...
        if (StrUtil.isNotBlank(holdName) && StrUtil.isNotBlank(holdContactsPhone)) {
            ...
            ThreadUtil.execute(() -> GenericRpcResolver.resolver(customerEmployeeRpc.createdByOpenCard(input), ServiceName.CUSTOMER));
        }
    }
}
